  ...
  login(credentials: ILoginCredentials): Observable<IAuthResponse> {
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => this.toAuthError(error, 'No fue posible iniciar sesión.'))
      );
  }

  register(payload: IRegisterPayload): Observable<IAuthResponse> {
    return this.http
      .post(`${API_BACKEND_BASE}/usuarios/guardar/CLIENTE`, {
        ...
        cuentasBancarias: [
          {
            nombreBanco: payload.bankName.trim(),
            cuenta: payload.bankAccount.trim(),
            tipoCuenta: payload.bankAccountType
          }
        ]
      })
      .pipe(
        ...
        catchError((error) => this.toAuthError(error, 'No fue posible crear la cuenta.'))
      );
  }
